...
include "../../web_db/connection.php";  // your DB connection file
    ...
    $stmt = $conn->prepare("INSERT INTO rentals(car_id, renter_full_name, id_number, telephone, price, rent_date, return_date, days_in_rent, total_fee,user_id)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
    ...
    if ($stmt->execute() && $setInRent->execute()){
        ...
        <div id='successAlertBox'>
            ✅ $car_name of Plate: $CarPlateNumber is Set in Rent Mode.
        </div>
          ...
          <script>
            document.addEventListener('DOMContentLoaded', function(){
              ...
              window.location.href='internal_status.php';
              }, 3000);
          });
          </script>
        ";
